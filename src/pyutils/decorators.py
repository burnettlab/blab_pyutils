...
def update_signature_from_partial(f_partial: partial) -> partial:
    ...
    bound_args = original_sig.bind_partial(
        *f_partial.args,
        **dict(
            chain.from_iterable(
                map(
                    lambda it: (
                        ([(it[0], {})] + list(it[1].items()))
                        if isinstance(it[1], dict)
                        else (it,)
                    ),
                    f_partial.keywords.items(),
                )
            )
        ),
    )
    ...
    for name, param in original_sig.parameters.items():
        if name not in list(bound_args.kwargs.keys()) + list(
            bound_args.arguments.keys()
        ):
            # Create a new parameter for the remaining ones
            new_param = param.replace(
                default=param.default
            )  # Retain default values if any
            remaining_params.append(new_param)

    # Create a new signature with only the remaining parameters
    new_sig = inspect.Signature(
        remaining_params, return_annotation=original_sig.return_annotation
    )
...
@wrap_once
def pickle_output(
    func: Optional[Callable[..., Any]] = None, *, date_format: str = "%Y%m%d_%H%M%S"
) -> Callable[..., Any]:
    ...
    @DetailedError
    def pickle_wrapper(
        *args, force_run: bool = False, check_time: bool = True, **kwargs
    ):
        inspect_f = inspect.unwrap(func)
        func_data_path = Path("data", f"{fname}")
        func_data_path.mkdir(parents=True, exist_ok=True)
        func_pkls = filter(
            lambda p: p.suffix == ".pkl" and p.stem.endswith("_input"),
            func_data_path.iterdir(),
        )
        ...
        while True:
            ...
            if check_time:
                ...
                mod_time = datetime.datetime.fromtimestamp(
                    Path(inspect.getfile(inspect_f)).stat().st_mtime
                )
                pkl_time = datetime.datetime.strptime(
                    "_".join(f_input.stem.split("_")[:-1]), date_format
                )
            ...
            if len(input_args) == len(args) and len(input_kwargs) == len(kwargs):
                if all(compare_args(*args) for args in zip(input_args, args)) and all(
                    k in input_kwargs and compare_args(input_kwargs[k], v)
                    for k, v in kwargs.items()
                ):
                    f_output = Path(
                        f_input.parent,
                        f"{f_input.stem.replace('_input', '_output')}.pkl",
                    )
                    assert (
                        f_output.exists()
...
@wrap_once
def redirect_output(
    func: Optional[Callable[..., Any]] = None, *, blocked: bool = False
) -> Callable[..., Any]:
    ...
    def redirect_wrapper(*args, verbose: bool = False, **kwargs):
        if not blocked and (
            verbose or all(f.function != func.__name__ for f in inspect.stack())
        ):
            ...
            std_err = os.devnull
            with (
                open(str(std_out), "w") as o,
                redirect_stdout(o),
                open(str(std_err), "w") as e,
                redirect_stdout(e),
            ):
