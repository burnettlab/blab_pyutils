...
class mplConfig(ABC):
    ...
    _default_params: Dict[str, Any] = field(
        default_factory=lambda: dict(mpl.rcParamsDefault)
    )
    ...
    def config_params(self) -> Dict[str, Any]:
        params = reduce(
            lambda d, o: dict(
                map(lambda k: (k, d.get(k, o.get(k))), set((*d.keys(), *o.keys())))
            ),
            map(
                lambda c: c().default_params,
                filter(lambda c: issubclass(c, mplConfig), type(self).mro()),
            ),
            {},
        )
