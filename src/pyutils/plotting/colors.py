...
GOOD_COLORS = {
    "Greys": lambda points: mpl.colormaps["Greys"](
        np.linspace(0.37, 0.75, 2 if points == 1 else points)[-points:]
    ),
    "Purples": lambda points: mpl.colormaps["Purples"](
        np.linspace(0.35, 0.825, 2 if points == 1 else points)[-points:]
    ),
    "Blues": lambda points: mpl.colormaps["Blues"](
        np.linspace(0.37, 0.875, 2 if points == 1 else points)[-points:]
    ),
    "Greens": lambda points: mpl.colormaps["Greens"](
        np.linspace(0.375, 0.9, 2 if points == 1 else points)[-points:]
    ),
    "Oranges": lambda points: mpl.colormaps["Oranges"](
        np.linspace(0.35, 0.93, 2 if points == 1 else points)[-points:]
    ),
    "Reds": lambda points: mpl.colormaps["Reds"](
        np.linspace(0.3, 0.9, 2 if points == 1 else points)[-points:]
    ),
}
end_all()
...
if __name__ == "__main__":
    ...
    custom_category = {
        "Custom": [
            colors.LinearSegmentedColormap.from_list(name=n, colors=GOOD_COLORS[n](256))
            for n in GOOD_COLORS.keys()
        ]
    }
    ...
    for cmap_category, cmap_list in list(custom_category.items()):
        ...
        for i, ax in enumerate(axs.flat):
            ...
            ax.xaxis.set_major_formatter(
                ticker.FixedFormatter(cmap_list[i * dsub : (i + 1) * dsub])
            )
