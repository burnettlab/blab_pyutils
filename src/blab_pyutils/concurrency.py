...
@public
def get_concurrency_executor(
    *args, max_threads: int, **kwargs
) -> Callable[..., cf.Executor]:
    ...
    """
    return partial(
        (cf.ThreadPoolExecutor if max_threads == 1 else cf.ProcessPoolExecutor),
        max_workers=(
            max_threads if max_threads > 1 else int(os.getenv("MAX_THREADS", 4))
        ),
    )
    ...
    def thread_wrapper(*args, multithread: bool = True, **kwargs):
        if multithread:
            max_threads = kwargs.get(
                "max_threads",
                globals().get("MAX_THREADS", int(os.getenv("MAX_THREADS", 4))),
            )
            for _ in filter(
                lambda f: any(
                    map(
                        lambda v: isinstance(v, (mp.Pool, cf.Executor)),
                        f.frame.f_locals.values(),
                    )
                ),
                inspect.stack(),
            ):
...
def concurrency(func: Callable[..., Any]) -> Callable[..., Any]:
    ...
    executor_arg = next(
        map(
            lambda p: p[0],
            filter(
                lambda p: issubclass(get_anno_class(p[1].annotation), cf.Executor),
    ...
    @max_threads
    @wraps(func)
    def concurrency_wrapper(
        *args, max_threads: int, executor: Optional[cf.Executor] = None, **kwargs
    ):
        if executor is None:
...
@public
def apply_with_kwargs(
    f: Callable[..., Any], args: Tuple[Any], kwargs: Dict[Any, Any]
) -> Any:
