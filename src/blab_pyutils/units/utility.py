...
def get_units(anno: inspect.Parameter):
    ...
    return next(
        map(get_units, getattr(anno, "__args__", [])),
        getattr(anno, "__unit__", getattr(anno, "units", None)),
    )


@public
def obj_using_units(
    obj: object, *args, USING_UNITS: Optional[bool] = None, **kwargs
) -> bool:
    ...
    if USING_UNITS is not None:
        return USING_UNITS
    return bool(
        getattr(obj, "USE_UNITS", globals().get("USE_UNITS", os.getenv("USE_UNITS")))
    )
...
@wrap_once
def arg_unit_conv(
    func: Callable[..., Any], *, pref_units: Optional[List[str]] = None
) -> Callable[..., Any]:
...
@public
@wrap_once
def return_unit_conv(
    func: Optional[Callable[..., Any]] = None, *, pref_units: Optional[List[str]] = None
) -> Callable[..., Any]:
...
def dynamic_unit_wrap(func: Callable[..., Any]) -> Callable[..., Any]:
    ...
    return_units_base = tuple(
        filter(None, anno_units(getargs(signature.return_annotation)))
    )
    ...
    def dynamic_unit_wrapper(*args, **kwargs):
        ...
        f_partial = update_signature_from_partial(
            partial(
                func,
                **dict(
                    map(
                        lambda k: (k, bound.arguments[k]),
                        bound.arguments.keys() - map(lambda p: p[0], pos_kw(signature)),
                    )
                ),
            )
        )
        ...
        arg_units = tuple(
            anno_units(map(lambda p: p.annotation, sig.parameters.values()))
        )

        # Wrap the call function for converting argument units
        first_arg = next(iter(bound.arguments.values()), None)
        if obj_using_units(first_arg, USING_UNITS=kwargs.pop("USING_UNITS", None)):
...
def unit_str_addon(unit: Union[NUM, UREG.Unit, PlainUnit]) -> str:
    ...
    return " [-]" if unit == UREG.dimensionless else rf" [{unit}]"
