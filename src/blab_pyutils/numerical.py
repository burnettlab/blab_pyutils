...
@public
def resample(
    x_i: npt.ArrayLike, y_i: npt.ArrayLike, num_points: int = 100, decade_th: int = 2
    ...
    xi_func = (
        np.linspace
        if np.nonzero(xi)[0].size != xi.size
        or (np.abs(np.log10(np.max(xi)) - np.log10(np.min(xi))) <= decade_th)
        else np.logspace
    )
...
def priority_interp(
    ...
    **kwargs,
    ...
    """
    assert getattr(x_, "units", None) == getattr(
        x_i, "units", None
    ), "x_ must have the same units as x_i"
    return_units = getattr(y_i, "units", 1)

    xi, yi, x = map(
        lambda arr: np.atleast_1d(getattr(arr, "magnitude", arr)), (x_i, y_i, x_)
    )
    unique_xi = np.unique(xi, return_index=True, axis=axis)[1]
    xi, yi = map(lambda a: a[unique_xi], (xi, yi))

    try:
        interpolator = interp.PchipInterpolator(
            xi, yi, axis=axis or 0, extrapolate=extrapolate
        )
...
@public
def local_min(
    x: npt.ArrayLike, y: npt.ArrayLike, num_points: int = 1000
) -> Tuple[npt.ArrayLike, npt.ArrayLike]:
...
@public
def local_extrema(
    x_: npt.ArrayLike, y_: npt.ArrayLike, max=True
) -> Tuple[npt.ArrayLike, npt.ArrayLike]:
    ...
    extrema_indices = np.where(
        (np.sign(dy[:-1]) != np.sign(dy[1:])) & (d2y < 0 if max else d2y > 0)
    )
    return x[extrema_indices].squeeze() * x_unit, y[extrema_indices].squeeze() * y_unit


@public
def symmetric_logscale(
    min_val: float, max_val: float, num_points: int = 100
) -> npt.ArrayLike:
...
@public
def auto_start(gen_func: Callable) -> Generator:
    """Wrapper to automatically start a generator on creation by sending `None`"""
